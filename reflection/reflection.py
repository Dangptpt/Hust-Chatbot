...
contextualize_q_prompt = """Dựa trên lịch sử câu hỏi và câu hỏi mới nhất của người dùng, hãy viết lại câu hỏi để có thể hiểu mà không cần đến các câu hỏi trước đó.
Lưu ý:
- KHÔNG trả lời câu hỏi
- Chỉ viết lại nếu THỰC SỰ cần thiết (câu hỏi có liên quan đến ngữ cảnh câu trước đó) , nếu không hãy in lại câu hỏi và không sửa đổi gì thêm
...
class LangchainReflection(BaseReflection):
    ...
    async def expand_query(self, question: str, chat_history: List[dict], num_context: int = 2) -> str:
        formatted_history = [
